...
function findGeminiVerbData(article,surface,reading){const verbs=getArticleGeminiVerbs(article);if(!verbs.length)return null;const s=normalizeGeminiKey(surface);const r=normalizeGeminiKey(reading);let exact=verbs.find(v=>normalizeGeminiKey(v.surface)===s&&(!r||normalizeGeminiKey(v.reading)===r));if(exact)return exact;return verbs.find(v=>normalizeGeminiKey(v.surface)===s)||null;}
function showDictPopup(el){const popup=document.getElementById('dict-popup');if(!popup)return;const alreadyActive=el.classList.contains('is-active');closeDictPopup();if(alreadyActive)return;const lang=getContentLanguage();const surface=(el.dataset.surface||'').trim();const lemma=(el.dataset.lemma||surface).trim();const rs=(el.dataset.readingSurface||'').trim();const rl=(el.dataset.readingLemma||'').trim();const showFormDetails=el.dataset.showFormDetails==='1';const form=(lang==='en'?el.dataset.formEn:el.dataset.formBg)||el.dataset.formBg||el.dataset.formEn||(lang==='en'?'form in context':'форма в текста');const formula=(lang==='en'?el.dataset.formulaEn:el.dataset.formulaBg)||el.dataset.formulaBg||el.dataset.formulaEn||'';let ms=(lang==='en'?el.dataset.meaningSurfaceEn:el.dataset.meaningSurfaceBg||el.dataset.meaningSurfaceEn||'').trim();const ml=(lang==='en'?el.dataset.meaningLemmaEn:el.dataset.meaningLemmaBg||el.dataset.meaningLemmaEn||'').trim();const labelForm=(lang==='en'?'Form':'Форма');const labelFormula=(lang==='en'?'Formation':'Образуване');const labelLemma=(lang==='en'?'Dictionary form':'Речникова форма');const missingLemmaMeaning=(lang==='en'?'no translation':'няма превод');const gemini=findGeminiVerbData(el.closest('article'),surface,rs);let geminiType='';let geminiFormation='';let geminiFormula='';if(gemini){ms=((lang==='en'?gemini.translation_en:gemini.translation_bg)||ms||'').trim();geminiType=((lang==='en'?gemini.form_type_en:gemini.form_type_bg)||'').trim();geminiFormation=((lang==='en'?gemini.formation_en:gemini.formation_bg)||'').trim();geminiFormula=((lang==='en'?gemini.formula_en:gemini.formula_bg)||'').trim();}let html='<div class="dw">'+surface+(rs?' ['+rs+']':'')+(ms?' - '+ms:'')+'</div>';if(gemini&&geminiType)html+='<div class="dm">'+labelForm+': '+geminiType+'</div>';else if(showFormDetails)html+='<div class="dm">'+labelForm+': '+form+'</div>';if(gemini&&geminiFormation)html+='<div class="dm">'+labelFormula+': '+geminiFormation+'</div>';else if(showFormDetails&&formula)html+='<div class="dm">'+labelFormula+': '+formula+'</div>';if(gemini&&geminiFormula)html+='<div class="dm">'+(lang==='en'?'Formula':'Формула')+': '+geminiFormula+'</div>';html+='<div class="dm">'+labelLemma+': '+lemma+(rl?' ['+rl+']':'')+' - '+(ml||missingLemmaMeaning)+'</div>';popup.innerHTML=html;el.classList.add('is-active');positionPopupNear(el,popup);}
