...
.shadow-sentence{display:inline-block;padding:2px 4px;border-radius:8px;transition:background-color .2s ease,box-shadow .2s ease}
.shadow-sentence.shadow-active{background:rgba(138,180,255,.38)!important;box-shadow:0 0 0 2px var(--accent)!important;color:var(--text)!important;display:inline-block}
...
function wrapBlockSentences(block){
  if(!block || block.dataset.shadowPrepared==='1') return Array.from(block.querySelectorAll('.shadow-sentence'));
  const html = block.innerHTML;
  const parts = html.split(/(?<=[。！？?!])/).filter(function(s){ return s && s.trim(); });
  if(!parts.length){
    block.dataset.shadowPrepared='1';
    return [];
  }
  block.innerHTML = parts.map(function(part){
    return '<span class="shadow-sentence">'+part+'</span>';
  }).join('');
  block.dataset.shadowPrepared='1';
...
function setupArticleShadowing(article){
  ...
  function buildTimings(){
    ...
    const weights = sentenceEls.map(function(el){ return Math.max(((el.textContent || '').trim()).length, 1); });
  ...
  function clearActive(){ sentenceEls.forEach(function(el){ el.classList.remove('shadow-active'); }); }
  function updateHighlight(){
    if(!timings.length) buildTimings();
    if(!timings.length) return;
    const t = audio.currentTime || 0;
    let current = timings.find(function(seg){ return t >= seg.start && t < seg.end; });
    if(!current){ current = (t >= (timings[timings.length - 1]?.end || 0)) ? timings[timings.length - 1] : timings[0]; }
    if(!current) return;
    clearActive();
